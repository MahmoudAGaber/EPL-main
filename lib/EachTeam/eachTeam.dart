...
class _EachTeamState extends State<EachTeam> with TickerProviderStateMixin {
              ...
              flexibleSpace: Consumer<EachTeamViewModel>(
               builder: (context,provider,child){
                 if (provider.headerProfileModel == null) {
                   ...
                   titlePadding: EdgeInsets.only(top: 1.0, bottom: 20.0,right: 100),
                   title: FittedBox(
                     child: Row(
                       mainAxisAlignment: MainAxisAlignment.start,
                       children: <Widget>[
                    ...
                    child:Consumer<EachTeamViewModel>(
                      builder: (context,provider,child){
                        return Container(
                          ...
                          child: tabController==null?Container()
                              :TabBar(
            ...
            body: SafeArea(
              child: NestedScrollView(
                ...
                body:  TabBarView(
                  ...
                  children: List.generate(tabView(provider).length, (index) => tabView(provider)[index]),),
              ),
            ),
          );
        },
  ...
  Widget overView(){
    return ListView(children: <Widget>[OverView(url: widget.url,)]);
  }
  Widget teamNews(){
    return ListView(physics: NeverScrollableScrollPhysics(),children:<Widget>[ TeamNews(url:widget.url,)]);
  }
  Widget videoTeam(){
    return ListView(children:<Widget>[ VideoTeam(url:widget.url,)]);
  }
  Widget matchesForTeam(){
   return  ListView(children: <Widget>[MatchesForTeam(url: widget.url,)]);
  }
  Widget teamPosition(){
    return ListView(children: <Widget>[TeamPosition(url:widget.url,id: widget.id,)],);
  }
  Widget teamSquad(){
    return  ListView(children: <Widget>[teamTeam(url: widget.url,)]);
  }
  Widget statisticsForTeam(){
    return ListView(children: <Widget>[statisticsForTeam()]);
  }
  Widget transferTeam(){
    return ListView(children: <Widget>[tranferTeam(url: widget.url,)]);
  }
  Widget cups(){
